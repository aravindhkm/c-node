...
public class CypherCache<K, V> {

  @Getter
  private final CacheType name;
  private final Cache<K, V> cache;

  CypherCache(CacheType name, String strategy) {
    this.name = name;
    this.cache = CacheBuilder.from(strategy).build();
  }

  CypherCache(CacheType name, String strategy, CacheLoader<K, V> loader) {
    this.name = name;
  ...
  public boolean equals(Object o) {
    ...
    CypherCache<?, ?> tronCache = (CypherCache<?, ?>) o;
