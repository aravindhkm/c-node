...
import static org.tron.protos.contract.Common.ResourceCode.CYPHER_POWER;
...
public class FreezeBalanceActuator extends AbstractActuator {
    ...
    switch (freezeBalanceContract.getResource()) {
      case BANDWIDTH:
        if (!ArrayUtils.isEmpty(receiverAddress)
          ...
          long oldNetWeight = accountCapsule.getFrozenBalance() / CYP_PRECISION;
          long newFrozenBalanceForBandwidth =
              frozenBalance + accountCapsule.getFrozenBalance();
          accountCapsule.setFrozenForBandwidth(newFrozenBalanceForBandwidth, expireTime);
          long newNetWeight = accountCapsule.getFrozenBalance() / CYP_PRECISION;
      ...
      case ENERGY:
        if (!ArrayUtils.isEmpty(receiverAddress)
          ...
          long oldEnergyWeight = accountCapsule.getEnergyFrozenBalance() / CYP_PRECISION;
          ...
          long newEnergyWeight = accountCapsule.getEnergyFrozenBalance() / CYP_PRECISION;
          increment = newEnergyWeight - oldEnergyWeight;
        }
        addTotalWeight(ENERGY, dynamicStore, frozenBalance, increment);
        break;
      case CYPHER_POWER:
        long oldTPWeight = accountCapsule.getTronPowerFrozenBalance() / CYP_PRECISION;
        ...
        long newTPWeight = accountCapsule.getTronPowerFrozenBalance() / CYP_PRECISION;
        increment = newTPWeight - oldTPWeight;
        addTotalWeight(CYPHER_POWER, dynamicStore, frozenBalance, increment);
        break;
      default:
  ...
  private void addTotalWeight(ResourceCode resourceCode, DynamicPropertiesStore dynamicStore,
    ...
    long weight = dynamicStore.allowNewReward() ? increment : frozenBalance / CYP_PRECISION;
    switch (resourceCode) {
      case BANDWIDTH:
  ...
  private long delegateResource(byte[] ownerAddress, byte[] receiverAddress, boolean isBandwidth,
    ...
    long newWeight;
    if (isBandwidth) {
      oldWeight = receiverCapsule.getAcquiredDelegatedFrozenBalanceForBandwidth() / CYP_PRECISION;
      receiverCapsule.addAcquiredDelegatedFrozenBalanceForBandwidth(balance);
      newWeight = receiverCapsule.getAcquiredDelegatedFrozenBalanceForBandwidth() / CYP_PRECISION;
    } else {
      oldWeight = receiverCapsule.getAcquiredDelegatedFrozenBalanceForEnergy() / CYP_PRECISION;
      receiverCapsule.addAcquiredDelegatedFrozenBalanceForEnergy(balance);
      newWeight = receiverCapsule.getAcquiredDelegatedFrozenBalanceForEnergy() / CYP_PRECISION;
    }
