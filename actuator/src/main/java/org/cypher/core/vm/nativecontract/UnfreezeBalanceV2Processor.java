...
public class UnfreezeBalanceV2Processor {
    ...
    switch (param.getResourceType()) {
      ...
      case CYPHER_POWER:
        if (dynamicStore.supportAllowNewResourceModel()) {
          if (!this.checkExistFrozenBalance(accountCapsule, Common.ResourceCode.CYPHER_POWER)) {
            throw new ContractValidateException("no frozenBalance(CYPHER_POWER)");
          }
        } else {
      ...
      default:
        if (dynamicStore.supportAllowNewResourceModel()) {
          throw new ContractValidateException("Unknown ResourceCode, valid ResourceCode[BANDWIDTH、ENERGY、CYPHER_POWER]");
        } else {
          throw new ContractValidateException("Unknown ResourceCode, valid ResourceCode[BANDWIDTH、ENERGY]");
        }
    }
  ...
  private void updateVote(
    ...
    // cypher power is enough to total votes
    if (ownedTronPower >= totalVote * CYP_PRECISION) {
      return;
    }
    ...
    for (Protocol.Vote vote : accountCapsule.getVotesList()) {
      long newVoteCount =
          (long) ((double) vote.getVoteCount() / totalVote * ownedTronPower / CYP_PRECISION);
      if (newVoteCount > 0) {
        votesToAdd.add(
