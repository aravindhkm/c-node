...
public class ManagerTest extends BlockGenerate {
  ...
  public void updateRecentTransaction() throws Exception {
    ...
    b.addTransaction(cyp);
    dbManager.updateRecentTransaction(b);
    try {
      dbManager.consumeBandwidth(cyp, new TransactionTrace(cyp, StoreFactory.getInstance(),
          new RuntimeImpl()));
    } catch (Exception e) {
      Assert.assertTrue(e instanceof ContractValidateException);
    }
    dbManager.consumeMemoFee(cyp, new TransactionTrace(cyp, StoreFactory.getInstance(),
        new RuntimeImpl()));
    Assert.assertTrue(dbManager.getTxListFromPending().isEmpty());
    Assert.assertNull(dbManager.getTxFromPending(cyp.getTransactionId().toString()));
    ...
    Assert.assertEquals(cyp.getTransactionId().toString(), item.getTransactionIds().get(0));
  }
  ...
  public void transactionTest() {
    ...
    ShieldContract.ShieldedTransferContract cyp1 = ShieldContract.ShieldedTransferContract
            ...
            .setToAmount(10)
            .build();
    TransactionCapsule trans = new TransactionCapsule(cyp1, ContractType.ShieldedTransferContract);
    try {
  ...
  public void testExpireTransaction() {
    ...
    cyp.setExpiration(latestBlockTime - 100);
    try {
      dbManager.validateCommon(cyp);
      Assert.fail();
