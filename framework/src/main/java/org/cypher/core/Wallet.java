...
import static org.tron.core.services.jsonrpc.CypherJsonRpcImpl.EARLIEST_STR;
...
public class Wallet {
  ...
  @Autowired
  private CypherNetService tronNetService;
  @Autowired
  ...
  private boolean cypCacheEnable = CommonParameter.getInstance().isCypCacheEnable();
  ...
  public TransactionCapsule createTransactionCapsule(com.google.protobuf.Message message,
    ...
    cyp.setTransactionCreate(false);
    if (contractType == ContractType.CreateSmartContract) {
  ...
  public GrpcAPI.Return broadcastTransaction(Transaction signedTransaction) {
    ...
    try {
      ...
      if (cyp.getInstance().getRawData().getContractCount() == 0) {
  ...
  public TransactionApprovedList getTransactionApprovedList(Transaction cyp) {
    ...
    if (cyp.getRawData().getContractCount() == 0) {
  ...
  public GrpcAPI.CanDelegatedMaxSizeResponseMessage getCanDelegatedMaxSize(
    ...
    if (canDelegatedMaxSize < CYP_PRECISION) {
  ...
  public long calcCanDelegatedBandWidthMaxSize(
    ...
    long netUsage = (long) (accountNetUsage * CYP_PRECISION * ((double)
  ...
  public long calcCanDelegatedEnergyMaxSize(ByteString ownerAddress) {
    ...
    long energyUsage = (long) (ownerCapsule.getEnergyUsage() * CYP_PRECISION * ((double)
  ...
  public AccountResourceMessage getAccountResource(ByteString accountAddress) {
    ...
    long allTronPower = accountCapsule.getAllTronPower() / CYP_PRECISION;
    ...
    builder.setFreeNetUsed(accountCapsule.getFreeNetUsage())
        ...
        .setTotalNetLimit(totalNetLimit)
        .setTotalNetWeight(totalNetWeight)
        .setTotalCypherPowerWeight(totalTronPowerWeight)
        .setEnergyLimit(energyLimit)
        .setEnergyUsed(accountCapsule.getAccountResource().getEnergyUsage())
        .setCypherPowerUsed(allTronPowerUsage)
        .setCypherPowerLimit(allTronPower)
  ...
  public NodeList listNodes() {
    ...
    if (!Args.getInstance().p2pDisable) {
      CypherNetService.getP2pService().getConnectableNodes().forEach(node -> {
  ...
  public Transaction triggerContract(TriggerSmartContract
    ...
    if (isConstant(abi, selector)) {
      return callConstantContract(cypCap, builder, retBuilder, false);
    } else {
      return cypCap.getInstance();
    }
  }
  ...
  public Transaction triggerConstantContract(TriggerSmartContract triggerSmartContract,
      ...
      throws ContractValidateException, ContractExeException, HeaderNotFound, VMIllegalException {
    return triggerConstantContract(triggerSmartContract, cypCap, builder, retBuilder, false);
  }

  public Transaction triggerConstantContract(TriggerSmartContract triggerSmartContract,
      TransactionCapsule cypCap, Builder builder, Return.Builder retBuilder, boolean isEstimating)
      throws ContractValidateException, ContractExeException, HeaderNotFound, VMIllegalException {
    ...
    return callConstantContract(cypCap, builder, retBuilder, isEstimating);
  }
  ...
  public Transaction callConstantContract(TransactionCapsule cypCap,
    ...
    TransactionContext context = new TransactionContext(headBlockCapsule, cypCap,
