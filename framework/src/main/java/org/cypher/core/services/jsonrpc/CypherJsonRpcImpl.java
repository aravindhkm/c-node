...
@Slf4j(topic = "API")
@Component
public class CypherJsonRpcImpl implements CypherJsonRpc, Closeable {
  ...
  @Autowired
  public CypherJsonRpcImpl(@Autowired NodeInfoService nodeInfoService, @Autowired Wallet wallet,
  ...
  private void estimateEnergy(byte[] ownerAddressByte, byte[] contractAddressByte,
    ...
    TransactionCapsule cypCap = wallet.createTransactionCapsule(triggerContract,
        ContractType.TriggerSmartContract);
    Transaction cyp =
        wallet.estimateEnergy(triggerContract, cypCap, cypExtBuilder, retBuilder, estimateBuilder);
    cypExtBuilder.setTransaction(cyp);
    cypExtBuilder.setTxid(cypCap.getTransactionId().getByteString());
    cypExtBuilder.setResult(retBuilder);
    retBuilder.setResult(true).setCode(response_code.SUCCESS);
  ...
  private String call(byte[] ownerAddressByte, byte[] contractAddressByte, long value,
    ...
    String result;
    if (cypExtBuilder.getTransaction().getRet(0).getRet().equals(code.SUCESS)) {
      ...
      byte[] resData = cypExtBuilder.getConstantResult(0).toByteArray();
  ...
  public String getStorageAt(String address, String storageIdx, String blockNumOrTag)
    ...
    if (EARLIEST_STR.equalsIgnoreCase(blockNumOrTag)
      ...
      storage.generateAddrHash(smartContract.getCypHash().toByteArray());
  ...
  public String estimateGas(CallArguments args) throws JsonRpcInvalidRequestException,
    ...
    if (cypExtBuilder.getTransaction().getRet(0).getRet().equals(code.FAILED)) {
      String errMsg = retBuilder.getMessage().toStringUtf8();

      byte[] data = cypExtBuilder.getConstantResult(0).toByteArray();
  ...
  private TransactionJson buildTriggerSmartContractTransaction(byte[] ownerAddress,
    ...
    try {
      ...
      Transaction cyp = wallet
          .triggerContract(build.build(), new TransactionCapsule(txBuilder.build()), cypExtBuilder,
              retBuilder);
      cyp = setTransactionPermissionId(args.getPermissionId(), cyp);
      cypExtBuilder.setTransaction(cyp);
    } catch (JsonRpcInvalidParamsException e) {
      throw new JsonRpcInvalidParamsException(e.getMessage());
    } catch (ContractValidateException e) {
    ...
    String jsonString = Util.printTransaction(cypExtBuilder.build().getTransaction(),
        args.isVisible());
    TransactionJson transactionJson = new TransactionJson();
