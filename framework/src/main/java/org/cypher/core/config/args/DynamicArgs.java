...
public class DynamicArgs {
  ...
  private void updateActiveNodes(Config config) {
    ...
    List<InetSocketAddress> activeNodes = CypherNetService.getP2pConfig().getActiveNodes();
    activeNodes.clear();
    activeNodes.addAll(newActiveNodes);
    logger.debug("p2p active nodes : {}",
        CypherNetService.getP2pConfig().getActiveNodes().toString());
  }
  ...
  private void updateTrustNodes(Config config) {
    ...
    List<InetAddress> trustNodes = CypherNetService.getP2pConfig().getTrustNodes();
    trustNodes.clear();
    ...
    logger.debug("p2p trust nodes : {}",
        CypherNetService.getP2pConfig().getTrustNodes().toString());
  }
