...
public class RelayService {
  ...
  public boolean checkHelloMessage(HelloMessage message, Channel channel) {
    ...
    try {
      ...
      if (flag) {
        CypherNetService.getP2pConfig().getTrustNodes().add(channel.getInetAddress());
      }
      return flag;
    } catch (Exception e) {
  ...
  private void connect() {
    for (InetSocketAddress fastForwardNode : fastForwardNodes) {
      if (!CypherNetService.getP2pConfig().getActiveNodes().contains(fastForwardNode)) {
        CypherNetService.getP2pConfig().getActiveNodes().add(fastForwardNode);
      }
    }
  }
  ...
  private void disconnect() {
    fastForwardNodes.forEach(address -> {
      CypherNetService.getP2pConfig().getActiveNodes().remove(address);
      CypherNetService.getPeers().forEach(peer -> {
