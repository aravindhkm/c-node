...
public class EffectiveCheckService {
  ...
  private void findEffectiveNode() {
    ...
    List<Node> tableNodes = CypherNetService.getP2pService().getConnectableNodes();
    ...
    Optional<Node> chosenNode = tableNodes.stream()
        ...
        .filter(node -> !CypherNetService.getP2pConfig().getActiveNodes()
    ...
    CypherNetService.getP2pService().connect(chosenNode.get(), future -> {
