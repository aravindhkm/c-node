...
import static org.tron.core.config.Parameter.NetConstants.MAX_CYP_FETCH_PER_PEER;
    ...
    synchronized (this) {
      ...
      invToFetch.forEach((item, time) -> {
        ...
        peers.stream().filter(peer -> {
          ...
          return t != null && now - t < TIMEOUT && invSender.getSize(peer) < MAX_CYP_FETCH_PER_PEER;
  ...
  class InvSender {
    ...
    public void sendInv() {
      send.forEach((peer, ids) -> ids.forEach((key, value) -> {
        if (peer.isRelayPeer() && key.equals(InventoryType.CYP)) {
          return;
        }
