...
import org.tron.core.net.message.CypherMessageFactory;
...
public class P2pEventHandlerImpl extends P2pEventHandler {
  ...
  private void processMessage(PeerConnection peer, byte[] data) {
    ...
    CypherMessage msg = null;
    MessageTypes type = null;
    try {
      msg = CypherMessageFactory.create(data);
      type = msg.getType();
      ...
      if (INVENTORY.equals(type)) {
        ...
        int count = peer.getPeerStatistics().messageStatistics.tronInCypInventoryElement
                .getCount(10);
        if (inventoryType.equals(Protocol.Inventory.InventoryType.CYP) && count > maxCountIn10s) {
