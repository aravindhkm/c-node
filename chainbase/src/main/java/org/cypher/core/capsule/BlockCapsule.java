...
public class BlockCapsule implements ProtoCapsule<Block> {
  ...
  public void addAllTransactions(List<TransactionCapsule> pendingCyps) {
    List<Transaction> list = pendingCyps.stream().map(TransactionCapsule::getInstance).collect(
        Collectors.toList());
    this.block = this.block.toBuilder().addAllTransactions(list).build();
    getTransactions().addAll(pendingCyps);
  }
