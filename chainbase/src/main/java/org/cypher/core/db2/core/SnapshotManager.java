...
import org.tron.core.db.CypherDatabase;
...
public class SnapshotManager implements RevokingDatabase {
  ...
  private void createCheckpoint() {
    CypherDatabase<byte[]> checkPointStore = null;
    boolean syncFlag;
    try {
  ...
  private CypherDatabase<byte[]> getCheckpointDB(String dbName) {
  ...
  private void checkV2() {
    ...
    for (String cp: cpList) {
      ...
      CypherDatabase<byte[]> checkPointV2Store = getCheckpointDB(cp);
  ...
  private void recover(CypherDatabase<byte[]> tronDatabase) {
